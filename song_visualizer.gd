...
func create_color_gradient():
	...
	gradient.add_point(0.0, Color(0.2, 1.0, 0.8))
	gradient.add_point(0.5, Color(0.6, 0.2, 1.0))
	gradient.add_point(1.0, Color(1.0, 0.2, 0.2))
...
func _physics_process(_delta: float) -> void:
	...
	for i in range(NUM_BARS):
		...
		bars[i].size.y = MIN_Y_HEIGHT + lerp(bars[i].size.y, magnitude * 1000 * 2, 0.2)
		bars[NUM_BARS * 2 - i - 1].size.y = lerp(bars[i].size.y, magnitude * 1000 * 2, 0.2)
